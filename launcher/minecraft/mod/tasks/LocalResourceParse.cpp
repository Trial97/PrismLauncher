...
Resources::Type identify(const QFileInfo& file)
{
    if (file.exists() && file.isFile()) {
        if (ModUtils::validate(file)) {
            ...
            return Resources::Type::Mod;
        }
        if (DataPackUtils::validateResourcePack(file)) {
            qDebug() << file.fileName() << "is a resource pack";
            return Resources::Type::ResourcePack;
        }
        if (TexturePackUtils::validate(file)) {
            qDebug() << file.fileName() << "is a pre 1.6 texture pack";
            return Resources::Type::TexturePack;
        }
        if (DataPackUtils::validate(file)) {
            qDebug() << file.fileName() << "is a data pack";
            return Resources::Type::DataPack;
        }
        if (WorldSaveUtils::validate(file)) {
            qDebug() << file.fileName() << "is a world save";
            return Resources::Type::World;
        }
        if (ShaderPackUtils::validate(file)) {
            qDebug() << file.fileName() << "is a shader pack";
            return Resources::Type::ShaderPack;
        }
