                         ...
                         ModPlatform::IndexedVersion& mod_version) -> Mod
{
    ...
    if (mod_pack.provider == Resources::Platform::Curseforge) {
...
void V1::updateModIndex(const QDir& index_dir, Mod& mod)
{
    ...
    switch (mod.provider.value()) {
        ...
        case (Resources::Platform::Modrinth):
            ...
            break;
        default:
            qCritical() << QString("Did not write file %1 because of an unknown provider!").arg(normalized_fname);
            return;
    }
...
auto V1::getIndexForMod(const QDir& index_dir, QString slug) -> Mod
{
    ...
    {  // Basic info
        ...
        mod.side = Resources::Side::fromString(stringEntry(table, "side"));
    ...
    {  // [update] info
        using Provider = Resources::Platform;
        ...
        if ((mod_provider_table = update_table[Provider(Provider::Curseforge).toString().toStdString()].as_table())) {
            mod.provider = Provider::Curseforge;
        ...
        if (deps) {
            for (auto&& depNode : *deps) {
                ...
                if (dep) {
                    ...
                    d.type = Resources::DependencyType::fromString(stringEntry(*dep, "type").toLower());
                    mod.dependencies << d;
                }
            }
        }
