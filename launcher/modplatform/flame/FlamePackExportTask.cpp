...
void FlamePackExportTask::collectHashes()
{
    ...
    for (const QFileInfo& file : m_files) {
        ...
        if (relative.startsWith("resourcepacks/") &&
            ...
            auto hashTask = Hashing::createHasher(file.absoluteFilePath(), Resources::Platform::Curseforge);
        ...
        if (auto modIter = std::find_if(allMods.begin(), allMods.end(), [&file](Mod* mod) { return mod->fileinfo() == file; });
            ...
            auto hashTask = Hashing::createHasher(mod->fileinfo().absoluteFilePath(), Resources::Platform::Curseforge);
    ...
    for (auto mod : resolvedFiles) {
        if (mod.isMod) {
            content += QString(TEMPLATE)
                           ...
                           .replace("{url}", ModPlatform::getMetaURL(Resources::Platform::Curseforge, mod.addonId).toHtmlEscaped())
